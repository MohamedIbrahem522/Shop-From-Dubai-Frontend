...
function getSocial() {
  try {
    return JSON.parse(localStorage.getItem("admin_social")) || {};
  } catch {
    return {};
  }
}
function updateSocialLinks() {
  ...
  if (s.facebook) {
    document
      .querySelectorAll(".hero-social a, .footer-social a, .contact-item")
      .forEach((el) => {
...
(async function loadData() {
  ...
  try {
    const [pRes, cRes, rRes] = await Promise.all([
      fetch(
        "https://shop-from-dubai-backend-production.up.railway.app/products",
      ),
      fetch(
        "https://shop-from-dubai-backend-production.up.railway.app/categories",
      ),
      fetch(
        "https://shop-from-dubai-backend-production.up.railway.app/reviews",
      ),
    ]);
    ...
    if (!cache) {
      try {
        ...
        allProducts = (await res.json()).map((p, i) => ({
          ...p,
          id: p._id || p.id || i,
        }));
      } catch {}
...
function renderCategories(cats) {
  ...
  catContainer.querySelectorAll("button").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      renderProducts(
        allProducts.filter(
          (p) => p.category?.trim() === btn.dataset.category.trim(),
        ),
      );
    });
  });
...
function openModal(product) {
  ...
  modalImg.onerror = () => {
    modalImg.src =
      "https://via.placeholder.com/400x400/f5f0e8/b8935a?text=Shop+from+Dubai";
  };
...
// ===== Hero Button =====
document
  .querySelector(".hero-btns .btn-primary")
